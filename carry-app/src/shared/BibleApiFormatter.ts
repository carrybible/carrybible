...
export const processHtml = (content: string, color: Color, canSelectVerse: boolean, backgroundColor: string): string => {
  return `<!DOCTYPE html>
  <html>
  ...
  <body style="background-color: ${backgroundColor ? backgroundColor : color.id === 'light' ? color.white : color.black}">
      <br/>
      <br/>
      <br/>
      <br/>
      ...
      const rnWebView = window.ReactNativeWebView || null;
      if (rnWebView) {
        ...
        rnWebView.postMessage(JSON.stringify({
          action: "scroll",
          data: {offsetY: window.pageYOffset, layoutHeight: window.innerHeight, maxScrollHeight },
        }));

      const l = document.getElementsByClassName('verse-span').length;

      if(${canSelectVerse}) { 
        for (let i = 0; i < l; i += 1) {
          const ele = document.getElementsByClassName('verse-span')[i];
          ele.addEventListener('click', evt => clickVerse(ele, evt));
        }
      }

      document.getRootNode().addEventListener('scroll', event => {
        const maxScrollHeight = Math.max(
          document.body.scrollHeight,
          document.body.offsetHeight,
          document.documentElement.clientHeight,
      ...
      function clickVerse(ele, evt) {
          ele.style.backgroundColor = '#aac2fe';
         
          const elements = document.getElementsByClassName('verse-span')
          var text = ""
          for (let i = 0; i < elements.length; i += 1) {
              if (elements[i].dataset.verseId === ele.dataset.verseId) {
                  if (elements[i].dataset.isSelecting === 'selected') {
                      elements[i].style.backgroundColor = 'transparent';
                      elements[i].dataset.isSelecting = undefined
                  }
                  else {
                      elements[i].style.backgroundColor = '#aac2fe';
                      elements[i].dataset.isSelecting = 'selected'
          ...
          rnWebView.postMessage(JSON.stringify({
            action: "clicked",
            data: ele.dataset.verseId,
            highlight: ele.dataset.isSelecting === 'selected',
            text: text,
          }));
        }
      }
