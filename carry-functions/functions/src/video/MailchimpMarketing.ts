...
export async function subscribeNewMember(audienceId: string, email: string, mergeFields: Record<string, any>, tags?: Array<string>) {
  ...
  await mailchimpMarketing.lists.addListMember(audienceId, body);
...
async function updateMemberData(audienceId: string, email: string, mergeFields: Record<string, any>) {
  const emailHash = createHash('md5').update(email).digest('hex')
  const body: mailchimpMarketing.UpdateListMemberBody = {
    merge_fields: mergeFields
  }

  await mailchimpMarketing.lists.updateListMember(audienceId, emailHash, body);
}

async function checkMember(audienceId: string, email: string) {

  try {
    const emailHash = createHash('md5').update(email).digest('hex');
    const member = await mailchimpMarketing.lists.getListMember(audienceId, emailHash);
